...
int x265_main(int argc, char **argv)
{
    ...
    // DEBUG
    char *paramStr = x265_param2string(param);
    DBG_STREAM("Before eccoder_parameters() param is: " << paramStr);
    x265_free( paramStr );
    /* get the encoder parameters post-initialization */
    api->encoder_parameters(encoder, param);
    // DEBUG
    paramStr = x265_param2string(param);
    DBG_STREAM("After eccoder_parameters() param is: " << paramStr);
    x265_free( paramStr );
...
// implement of network services

void DesktopStreamingService::DoStartEncoder( const std::string &cmd )
{
    using namespace std;
    ...
    for( char *p = strtok(const_cast<char*>(cmd.c_str()), SPACES); p; p = strtok(NULL, SPACES) )
        cmdArgs.push_back(p);

    int argc = (int)(cmdArgs.size());
    char **argv = &cmdArgs[0];
